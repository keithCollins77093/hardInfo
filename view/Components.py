...
#   Purpose:        Custom and composite GUI components not supplied in tkinter or tkinter.ttk.
#   Development:
#       2021-07-15:
...
#
#       2022-03-18:
#           Removing all code not needed for hardInfo project.
#
...
class JsonTreeView(ttk.Treeview):
    ...
    def __init__(self, container, jsonContent: dict, jsonTreeviewConfig: dict):
        ...
        self.listener = None
        if 'listener' in jsonTreeviewConfig:
            if callable(jsonTreeviewConfig['listener']):
                self.listener = jsonTreeviewConfig['listener']

        openBranches = None
        if "openBranches" in jsonTreeviewConfig:
            openBranches = jsonTreeviewConfig["openBranches"]
        ...
        mode = None
        if "mode" in jsonTreeviewConfig:
            mode = jsonTreeviewConfig["mode"]
        ...
        if 'columnWidths' in jsonTreeviewConfig:
            if isinstance(jsonTreeviewConfig['columnWidths'], dict):
                if 'name' in jsonTreeviewConfig['columnWidths']:
                    if isinstance(jsonTreeviewConfig['columnWidths']['name'], int):
                        self.nameColWidth = jsonTreeviewConfig['columnWidths']['name']
                if 'value' in jsonTreeviewConfig['columnWidths']:
                    if isinstance(jsonTreeviewConfig['columnWidths']['value'], int):
                        self.valueColWidth = jsonTreeviewConfig['columnWidths']['value']
            else:
    ...
    def addBranch(self, parentId, name, branches):
        if isinstance(branches, dict):
            if self.mode == JsonTreeView.MODE_STRICT:
                branchId = self.insert(parentId, END, text=str(name), open=self.openBranches)
            elif self.mode == JsonTreeView.MODE_NAME_VALUE:
                branchId = self.insert(parentId, END, values=(str(name), ""), open=self.openBranches)
            ...
            if name == None:
                if self.mode == JsonTreeView.MODE_STRICT:
                    self.insert(parentId, END, text=str(branches), open=self.openBranches)
                elif self.mode == JsonTreeView.MODE_NAME_VALUE:
                    self.insert(parentId, END, values=(str(branches), ""), open=self.openBranches)
            else:
                if self.mode == JsonTreeView.MODE_STRICT:
                    branchId = self.insert(parentId, END, text=str(name), open=self.openBranches)
                    self.insert(branchId, END, text=str(branches), open=self.openBranches)
                elif self.mode == JsonTreeView.MODE_NAME_VALUE:
                    branchId = self.insert(parentId, END, values=("\t"+str(name), str(branches)),
...
class JsonTreeViewFrame(LabelFrame):
    ...
    def __init__(self, container, jsonContent: dict, jsonTreeviewConfig: dict, **keyWordArguments):
        ...
        self.horzScrollbar = ttk.Scrollbar(self, orient=HORIZONTAL, command=self.jsonTreeview.xview)
        self.vertScrollbar = ttk.Scrollbar(self, orient=VERTICAL, command=self.jsonTreeview.yview)
        self.jsonTreeview.configure(yscroll=self.vertScrollbar.set, xscroll=self.horzScrollbar.set)

        self.jsonTreeview.pack(expand=True, fill=BOTH, anchor=W)
        self.vertScrollbar.pack(expand=True, fill=Y, anchor=E, side=RIGHT)

        #   self.syntaxTreeview.grid(row=0, column=0, sticky=NSEW, rowspan=3)
        #   self.vertScrollbar.grid(row=0, column=1, sticky=NS, rowspan=3)
        #   self.horzScrollbar.grid(row=3, column=0, sticky=EW)
...
class TtkTogleButton(ttk.Label):
